    ...
    <a-layout-sider width="260" style="background: #fff; min-height: 500px" class="pb-16">
      ...
      <a-menu v-model:selectedKeys="selectedKeys" v-model:openKeys="openKeys" class="font-normal" mode="inline">
        ...
        <a-sub-menu key="gcbmUpload" class="font-normal" :disabled="!title">
          <template #title>
            <span class="flex items-center text-lg">Submit Dataset <UploadOutlined class="mx-2" /></span>
          </template>
        ...
        <a-sub-menu key="gcbmConfigParams" class="font-normal" :disabled="!classifiersUploaded">
          <template #title>
            <span class="flex items-center text-lg">Configure Parameters <EditOutlined class="mx-2" /></span>
          </template>
        ...
        <!-- VIZ -->
        <a-menu-item key="gcbmViz" @click="() => onMenuItemClick('gcbmViz')" :disabled="!classifiersUploaded">
          <span class="flex items-center text-lg">
            Visualization
            <LineChartOutlined class="mx-2" />
          </span>
        </a-menu-item>

        <!-- DOWNLOAD -->
        <a-menu-item key="gcbmDownload" @click="() => onMenuItemClick('gcbmDownload')" :disabled="!classifiersUploaded">
          <span class="flex items-center text-lg">
            Download Simulation
            <DownloadOutlined class="mx-2" />
          </span>
        </a-menu-item>
      </a-menu>
    </a-layout-sider>
  ...
  setup() {
    ...
    watchEffect(() => {
      ...
      if (path === 'gcbm/create') {
        ...
        selectedKeys.value = ['gcbmViz']
        openKeys.value = []
      } else if (path === 'gcbm/download') {
        selectedKeys.value = ['gcbmDownload']
        openKeys.value = []

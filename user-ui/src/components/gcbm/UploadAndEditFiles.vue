...
export default {
  ...
  setup(props) {
    ...
    const beforeUpload = (file) => {
      ...
      const isValid = props.acceptedFileExtension.includes(fileExtension);

      if (!isValid) {
        message.error({
          content: `${file.name} is not a valid file type. You can only upload ${props.acceptedFileExtension.join(', ')} files.`,
          duration: 5
        })
        return false
      }

...
export default {
  ...
  setup() {
    ...
    function onStartDateChange(date) {
      var start_date_value = new Date(dayjs(date).format('YYYY-MM-DD'))
      store.commit('setGCBMLocalDomainState', { key: 'START_DATE', newState: start_date_value })
    }

    function onEndDateChange(date) {
      var end_date_value = new Date(dayjs(date).format('YYYY-MM-DD'))
      store.commit('setGCBMLocalDomainState', { key: 'END_DATE', newState: end_date_value })
    }

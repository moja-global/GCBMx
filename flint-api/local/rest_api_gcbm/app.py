...
from flask import Flask, send_from_directory, request, jsonify, redirect, send_file, Response
...
@app.route("/gcbm/logs/<title>", methods=["GET"])
def gcbm_logs(title):
    title = "".join(c for c in title if c.isalnum())
    log_file_path = os.path.join(os.getcwd(), 'input', title, 'gcbm_logs.csv')
...
def launch_run(title, input_dir):
    ...
    output_dir = f"{os.getcwd()}/output/{title}"
    with open(f"{input_dir}/gcbm_logs.csv", "w") as f:
        ...
        os.makedirs(f"{output_dir}/aspatial", exist_ok=True)
        logging.debug("Compiling spatial results")
        res2 = subprocess.Popen(
            [
                "python",
                "create_tiffs.py",
                output_dir,
                f"{output_dir}/spatial",
            ],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
        ...
        res2.wait()
        logging.debug("Communicating")
        (output, err) = res2.communicate()
        logging.debug("Communicated")

        logging.debug("Compiling databse results")
        res3 = subprocess.Popen(
            [
                "python",
                "compileresults.py",
